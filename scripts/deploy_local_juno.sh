...
docker cp artifacts/cw_721_base_starter.wasm $CONTAINER_NAME:/cw_721_base_starter.wasm
...
# upload howl wasm
CONTRACT_CODE=$($BINARY tx wasm store "/cw_721_base_starter.wasm" --from validator $TXFLAG --output json | jq -r '.logs[0].events[-1].attributes[0].value')
...
INIT="{\"name\": \"Juno NFT contract\", \"symbol\": \"JUNO_NFT\",\"minter\": \"$($BINARY keys show "validator" -a)\"}"
echo "$INIT" | jq .
$BINARY tx wasm instantiate $CONTRACT_CODE "$INIT" --from "validator" --label "cw_721_base_starter" $TXFLAG --no-admin
RES=$?
